...
- Auto-paired BRAWL on every mint (50 BRAWL + $10 ETH into lpTreasury)
- Bulk mint discount (5+ gets 1 free, 10+ gets 3 free, 20 gets 7 free)
- Lottery extra mint (1-in-2000 chance per paid mint)
- Founder fight discount (50% off for token IDs 1 to 100)
- Free first resurrect for founder tokens 1 to 100
...
**One-click boot**: `boot.bat` at repo root opens two windows. One running
the bot daemon (auto-restart on crash), one for Claude Code (run
`claude resume` to pick this conversation, or `claude` for a fresh one).
Pin to taskbar for one-click reboot recovery. `stop-bots.bat` kills the bots.

**Marketing art shipped 2026-04-28**: `marketing/art/` has 6 images
generated via Pollinations (free, no key, browser UA needed to bypass the
gate). Files: `pfp.png` (1024² king-on-throne), `x-banner.png` (1500×500
arena scene), `tg-header.png` (1024×576 5-brawler lineup), `tg-pfp-square.png`
(640² king close-up), `founder-celebration.png`, `graveyard.png`.
Re-run `python3 marketing/art/gen.py` to regenerate.

**Marketing kit**: `marketing/` directory holds the full pre-launch + launch
+ week-1 ops kit:
...
3. Watch 24 to 48 hours, blacklist obvious bots.
...
- **CLI `mint-onchain`** still calls `brawlers.mint()` directly. Works on
  Anvil (PLAYER_KEY == owner), reverts on BSC Testnet. Rewrite it to use
...
- `mint/page.tsx` reads `MintDrop.batchCost(count)` for tx value. Falls back
  to flat `ethPrice * count` when v4 contract reverts the read.
- `dash/SettingsEditors.tsx` has new `FounderDiscountEditor` and
  `TierPricingEditor`. Both render a "needs v5+" hint until the read succeeds.
- `lib/abi.ts` DUEL_ABI swaps `FOUNDER_DISCOUNT_BPS` const for
  `founderDiscountBps` state + `setFounderDiscountBps`. MINTDROP_ABI adds
  `priceTierCount`, `priceTierAt`, `priceForMint`, `batchCost`,
  `setPriceTiers`.

Keys live in `/c/tools/brawlers/.env.base-sepolia` (gitignored). Same wallet
carries through to Base mainnet (chain 8453).

**RPC note**: use `https://base-sepolia-rpc.publicnode.com`. The
`sepolia.base.org` endpoint is heavily rate-limited and breaks both wallet
broadcast and dapp reads.
...
- `jsx: "preserve"` becomes `jsx: "react-jsx"`
- `.next/dev/types/**/*.ts` gets added to `include`

Harmless. **DO NOT** put `".."` or `"../**/*"` in `exclude`. TypeScript
normalises those paths and TS18003 blows up. Use `rootDir: "."` if you need
...
`config.chains[0].id` when the wallet is on an unconfigured chain. Use
`useAccount().chainId` (the connector's actual current chain) instead.
...
`.env.local` also holds `BRAWLERS_SIGNER_KEY`. It's a Node-runtime env var
(no `NEXT_PUBLIC_` prefix). Only `/api/run-duel` reads it. On Vercel, set as
a **production** env var via `vercel env add`. NEVER commit.

### Vercel CLI quirks

- `vercel env add` needs `--value "X" --yes` for non-interactive use.
...
BSC Testnet returns `tBNB`. BSC mainnet returns `BNB`. Everything else
(Anvil, Base, Ethereum) returns `ETH`.

### Next.js security patches

Pin Next to the latest 16.0.x patch. CVE-2025-66478 affects <16.0.7.
Currently on 16.0.10. Bump when a new patch is released.
...
above all. Internal contract tiers stay 0 to 4 for the shuffled drop, plus
5 for king. The label swap is UI-only in `frontend/src/lib/rarity.ts` and
...
- **Rarity colours**: common grey, uncommon blue, rare purple, legendary
  orange, epic orange
- **All-purple background per brawler** (`#4A2C7A` to `#6A3AA8`), applied
...
Phase 4 established byte-identical behaviour between:
...
Long-term: refactor into an npm workspace or shared package so there's one
...
deployer or signer key without explicit authorisation**. Use `cast call`
(eth_call) to simulate when verifying behaviour. `cast send` needs a green
light.
