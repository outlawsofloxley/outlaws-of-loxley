...
`getting-started` → wallet, eth, first connect.
`minting` → tiers, founder perks, rarity, bulk discount, lottery.
`brawlers` → stats, weapons, the king.
`duelling` → how a fight works, stakes, payout, combat mechanics, rating.
`death-and-resurrection` → 3 strikes, the cost curve, the graveyard.
`marketplace` → list, buy, fee, trade.
`house-fighters` → why there's always someone to fight.
