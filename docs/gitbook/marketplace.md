...
3. enter a price in eth. the site calculates the 5% dev fee and shows you the net you'd receive.
...
3. **5% of the sale goes to the dev treasury, 95% to the seller.**
...
the marketplace contract takes 5% of every sale and routes it to the dev treasury. that pays for ongoing development, the keeper bot infrastructure, the discord, the website hosting, and any future seasons.

5% is on the lower end for nft marketplaces. opensea is 2.5%. blur is 0%. magic eden is variable. we picked 5% because we run all the infra ourselves and the platform doesn't have other revenue past the initial mint.
...
if you sell on opensea, opensea takes their 2.5%, no royalty enforcement on most non-base markets. our 5% only fires on the in-app marketplace. trade-off: external markets are cheaper for the seller, but the buyer doesn't get the in-app trust signals (live duel history, rating curve, etc).
